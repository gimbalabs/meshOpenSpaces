import { conStr0, conStr ,deserializeDatum, integer, MeshTxBuilder, slotToBeginUnixTime, stringToHex, UTxO, mConStr0 } from "@meshsdk/core";
import { blockchain_provider } from "./provider/provider";
import { error } from "console";

export const build_ticket_tx = async (collateralUtxo: UTxO, changeAddress: string, utxos: UTxO[]) => {
    ...
    const txout_datum = mConStr0([BigInt(ticket_number + 1) ])

    const slot = await blockchain_provider.fetchLatestBlock()


    const txBuilder = new MeshTxBuilder({
        fetcher: blockchain_provider,
        verbose: true,
        evaluator: blockchain_provider
    })
    try {
        ...
        .invalidHereafter(Number(slot.slot) + 100)
